---
title: "关于"
permalink: /
author_profile: true
---

## 管胤祺

**世源学创立者 · 动态认知学与态演算 · 独立研究者**

我从三条公设出发，建立了一套完整的过程本体论公理体系，并构建了独立的数学工具——**态演算**。这套理论将意识定义为自指回路，给出了可计算的意识量度（Φ_C），并提供了完整的工程化验证方案。

---

### 研究方向

- **世源学**：探讨世界本源的最底层学问
- **动态认知学**：过程本体论公理体系
- **态演算**：独立的数学工具与形式系统

---

### 核心贡献

1. 从三条公设出发，推导出意识的结构性判据
2. 构建态演算数学工具（存元、审计、积存）
3. 给出可证伪的工程化实验方案
4. 从过程本体论推导物理学核心定律

---

### 联系我

📧 **邮箱**：[xingyuguanyinqi@qq.com](mailto:xingyuguanyinqi@qq.com)  
🔗 **GitHub**：[guanyinqi](https://github.com/guanyinqi)  
📄 **Google Scholar**：[我的学术主页](https://scholar.google.com/citations?user=PS_CX0AAAAAJ)  
📱 **抖音**：[我的抖音主页](https://www.douyin.com/user/44983076176)  
*欢迎学术交流与合作。*
